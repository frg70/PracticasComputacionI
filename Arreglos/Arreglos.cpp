...
array<float, LEN> LlenarArreglo(){
    ...
    for (int i = 0; i < arreglo.size(); i++){
        cout << "Valor elemento [" << i << "]:" ; //Se define el ciclo
        cin >> arreglo[i]; //se da entrada a los valores
        }
    return arreglo;
}
...
array<float, LEN> MultiplicarArreglo(array<float, LEN> arreglo, float multiplo){
    for (int i = 0; i < arreglo.size(); i++) {
        arreglo[i] = multiplo * arreglo[i]; //se realiza la multiplicación de valores con un ciclo
        }
    return arreglo;
}
...
array<float, LEN> SumarArreglos(array<float, LEN> arr1, array<float, LEN> arr2)
{
    array<float, LEN> arreglo{0};
    for (int i = 0; i < arreglo.size(); i++){
        arreglo[i] = arr1[i] + arr2[i]; //se realiza la suma elemento a elemento
        }
        return  arreglo;
}